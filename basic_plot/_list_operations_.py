...
def make_listOfList(inp):
    if not (_check_.is_listOfList(inp)):
        return [inp]
    else:
        return inp

def make_listOfNpArray(inp):
    if not (_check_.is_listOfNpArray(inp)):
        return [inp]
    else:
        return inp

def make_listOfList_or_listOfNpArray(inp):
    if not (_check_.is_listOfList(inp) or _check_.is_listOfNpArray(inp) or 
            _check_.is_list_containing_lists_or_npArray(inp)):
        return [inp]
    else:
        return inp
